...
fun VodListScreen(typeId: String, title: String, onOpenVod: (vodId: String, name: String) -> Unit) {
    ...
    LaunchedEffect(typeId) {
        vods = withContext(Dispatchers.IO) {
            val api = SourceRepo(ctx).firstApi()
            try { if (api.isEmpty()) emptyList() else Maccms(api).category(typeId, 1) } catch (e: Exception) { emptyList() }
        }
        loading = false
    }
    ScreenScaffold(title = title, loading = loading, empty = vods.isEmpty(), emptyText = "该分类暂无片源") {
        items(vods) { v ->
            ListRow(label(vod = v)) { onOpenVod(v.vodId, v.vodName) }
        }
    }
}

internal fun label(vod: Vod): String =
